"""Tests for episodic memory and sleep cycle integration (pure-Python, no numpy)."""

import unittest
from unittest.mock import AsyncMock, MagicMock

from lucy_core._linalg import randn, norm
from lucy_core.memory.hippocampal import (
    HippocampalIndexer,
    EpisodicBuffer,
    EpisodicRecord,
    create_episodic_memory,
)
from lucy_core.sleep.orchestrator import (
    SleepOrchestrator,
    NREMReplay,
    REMSimulation,
    run_sleep_cycle,
...
class TestHippocampalIndexer(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_autoencode_reconstruction(self):
        ...
        self.assertLess(_mse(x, recon), 1.0)
    ...
    async def test_retrieval(self):
        ...
        mem1 = EpisodicRecord(
            ...
            trust_metric=0.9,
            sensory_features=x1,
            contextual_features=[0.0] * 512,
            abstract_features=[0.0] * 512,
        )
        indexer.store(mem1)

        # Retrieve with similar query
        query = _add_scaled(x1, randn(256), 0.05)
    ...
    async def test_nrem_replay_runs(self):
        ...
        class FakeMemory:
            def __init__(self):
                ...
                self.sensory_features = randn(256)
                self.contextual_features = randn(512)
                self.abstract_features = randn(512)
...
class TestSleepOrchestrator(unittest.IsolatedAsyncioTestCase):
    async def test_sleep_cycle(self):
        ...
        buffer.add({
            ...
            "sensory_features": randn(256),
            "contextual_features": randn(512),
            "abstract_features": randn(512),
